...
export default function ProductList() {
  ...
  // Extract the 'data' array from the received data
  ...
  const handleSort = (e, option) => {
    const sort = { _sort: option.sort, views: option.views };
    console.log({ sort });
    setSort(sort);
  };
  ...
  useEffect(() => {
    ...
    const actionPayload = { filter, sort, pagination };
    console.log('Dispatching action with payload:', actionPayload);
    dispatch(fetchProductsByFiltersAsync(actionPayload));
    ...
    <div className="bg-white">
      <div className="mx-auto max-w-2xl px-4 py-0 sm:px-6 sm:py-0 lg:max-w-7xl lg:px-8">
        ...
        {status === 'loading' ? (
          ...
          {products && Array.isArray(products.data) && products.data.map((product) => (
